/**
 * نظام المخطط الجامعي الاحترافي - النسخة النهائية المستقرة
 * تدعم: صمام أمان للبيانات القديمة، التاريخ الهجري، دمج المقررات، والملاحظات
 */

// 1. صمام الأمان: تحميل البيانات والتأكد من وجود كافة الحقول لمنع التعليق
const getInitialData = () => {
    const raw = JSON.parse(localStorage.getItem('uniCalendarData')) || {};
    return {
        config: raw.config || { startDate: '', endDate: '' },
        courses: raw.courses || [],
        holidays: raw.holidays || [],
        periods: raw.periods || [],
        procedures: raw.procedures || [], // الحقل الذي كان يسبب التعليق
        events: raw.events || []
    };
};

let universityData = getInitialData();
...
// --- محرك رندرة التقويم (Rendering Engine) ---
...
async function renderCalendar() {
    try {
        ...
        // العودة ليوم الأحد للأسبوع الأول
        ...
        while (current <= last) {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                if (current.getDay() < 5) { // عرض الأيام الدراسية فقط
        ...
        console.error("خطأ في الرندرة:", e);
        hideLoading();
    }
}
...
// --- إدارة البيانات والصفوف ---

function isTypeMatch(date, type) {
    if (!universityData[type]) return false;
    return universityData[type].some(i => date >= i.start && date <= i.end);
}

function getTypeTags(date, type) {
    if (!universityData[type]) return '';
...
document.getElementById('settingsForm').onsubmit = (e) => {
    ...
    universityData.courses = Array.from(document.querySelectorAll('#coursesList > div')).map(row => ({
        id: row.dataset.id || 'c_' + Math.random().toString(36).substr(2, 9),
...
window.showEventDetail = (id) => {
    ...
    document.getElementById('deleteEvent').onclick = () => {
        ...
        saveData(); toggleModal(detailModal, false);
    };
...
document.getElementById('exportCourse').onclick = () => {
    ...
    if(!course) return alert('اختر مقرر');
    downloadJSON({ type: 'course_package', course, events: universityData.events.filter(ev => ev.courseId == cid) }, `مواعيد_${course.name}.json`);
};
    ...
    reader.onload = (event) => {
        try {
            ...
            if (universalFilePicker.dataset.mode === 'full') {
                universityData = { ...getInitialData(), ...imported };
                saveData(); location.reload();
            } else {
                if(imported.type !== 'course_package') return alert('ملف غير صالح');
...
function setupDateConstraint(s, e) { if(s && e) s.onchange = () => { e.min = s.value; if(e.value < s.value) e.value = s.value; }; }
